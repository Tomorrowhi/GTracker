 ...
 * <p>GIF source can be specified in XML or by calling {@link #setInputSource(pl.droidsonroids.gif.InputSource)}</p>
 * <pre>
 *     &lt;GifTextureView
...
public class GifTextureView extends TextureView {
    ...
    private pl.droidsonroids.gif.InputSource mInputSource;
    ...
    private static pl.droidsonroids.gif.InputSource findSource(final TypedArray textureViewAttributes) {
        ...
        if (value.resourceId != 0) {
            ...
            if ("drawable".equals(type) || "raw".equals(type)) {
                return new pl.droidsonroids.gif.InputSource.ResourcesSource(textureViewAttributes.getResources(), value.resourceId);
            } else if (!"string".equals(type)) {
                throw new IllegalArgumentException("Expected string, drawable or raw resource, type " + type + " " +
                        "cannot be converted to GIF");
            }
        }
        return new pl.droidsonroids.gif.InputSource.AssetSource(textureViewAttributes.getResources().getAssets(), value.string.toString());
    }

    private class RenderThread extends Thread implements SurfaceTextureListener {

        final pl.droidsonroids.gif.ConditionVariable isSurfaceValid = new pl.droidsonroids.gif.ConditionVariable();
        private pl.droidsonroids.gif.GifInfoHandle mGifInfoHandle = pl.droidsonroids.gif.GifInfoHandle.NULL_INFO;
     ...
     */
    public synchronized void setInputSource(@Nullable pl.droidsonroids.gif.InputSource inputSource) {
        mRenderThread.dispose();
    ...
    /**
     * Equivalent of {@link pl.droidsonroids.gif.GifDrawable#setSpeed(float)}
     *
     ...
     * @see pl.droidsonroids.gif.GifDrawable#setSpeed(float)
     */
    ...
    /**
     * Returns last {@link IOException} occurred during loading or playing GIF (in such case only {@link pl.droidsonroids.gif.GifIOException}
    ...
    public IOException getIOException() {
        if (mRenderThread.mIOException != null) {
            ...
            return pl.droidsonroids.gif.GifIOException.fromCode(mRenderThread.mGifInfoHandle.getNativeErrorCode());
        }
    }
    ...
    private void updateTextureViewSize(final pl.droidsonroids.gif.GifInfoHandle gifInfoHandle) {
     ...
     * {@link pl.droidsonroids.gif.GifImageView}.
     *
    ...
    public Parcelable onSaveInstanceState() {
        ...
        return new pl.droidsonroids.gif.GifViewSavedState(super.onSaveInstanceState(), mFreezesAnimation ? mRenderThread.mSavedState : null);
    }

    @Override
    public void onRestoreInstanceState(Parcelable state) {
        pl.droidsonroids.gif.GifViewSavedState ss = (pl.droidsonroids.gif.GifViewSavedState) state;
