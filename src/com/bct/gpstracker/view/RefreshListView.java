...
public class RefreshListView extends ListView
{
    ...
    public OnRefreshDataListener getListener() {
        return listener;
    }
	...
	private void refreshState() {
		switch (currentState) {
		case PULL_DOWN:// 下拉刷新
			tv_state.setText(context.getString(R.string.drop_down_list_header_pull_text));
			iv_arrow.startAnimation(down_ra);
            tv_time.setVisibility(GONE);
			break;
		case RELEASE_STATE:// 松开刷新
		    tv_state.setText(context.getString(R.string.drop_down_list_header_release_text));
		    iv_arrow.startAnimation(up_ra);
            //设置刷新时间为当前时间
            tv_time.setVisibility(VISIBLE);
            tv_time.setText(getLastReFreshInfo());
			break;
...
//			tv_state.setText("正在刷新数据");
    ...
    private String getLastReFreshInfo() {
        if(lastRefreshTime==0){
            return null;
        }
        long curr = System.currentTimeMillis();
        long tm = (curr - lastRefreshTime) / 1000;
        String info = "上次刷新：";
        if (tm < 60) {
            info += tm + "秒前";
        } else if (tm < 3600) {
            ...
            info += format.format(lastRefreshTime);
        }
        return info;
    }
