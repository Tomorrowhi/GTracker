...
public class CrashHandler implements Thread.UncaughtExceptionHandler {
    ...
    private boolean handleException(Throwable ex) {
        ...
        new Thread() {
            ...
            public void run() {
                Looper.prepare();
                ExceptionCatchService.getInstance().sendError(exFileName + "");
                Looper.loop();
            }
        }.start();
    ...
    private String saveCrashInfo2File(Throwable ex) {
        ...
        String causedby = "exception=" + result.substring(result.lastIndexOf("Caused by:"));
        sb.append(causedby);
        FileOutputStream fos = null;
        try {

            String fileName = time + ".properties";
            if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                String path = mContext.getFilesDir() + "/crashLog/";
